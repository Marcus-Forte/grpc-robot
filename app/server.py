...
class RobotControlServicer(control_pb2_grpc.RobotControlServicer):
    ...
    def Move(self, request, context):
        ...
        """
        logging.info(
            f"Received Move command: direction={request.direction}, duration={request.duration}"
        )
...
def serve(robot: RobotControl):
    ...
    control_pb2_grpc.add_RobotControlServicer_to_server(
        RobotControlServicer(robot, command_queue),
        server,
    )
    ...
    worker = threading.Thread(
        target=process_commands(command_queue, stop_event),
        daemon=True,
    )
    worker.start()
